...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='utilisateurs' AND COLUMN_NAME='role') THEN
    ALTER TABLE utilisateurs ADD COLUMN role VARCHAR(20) DEFAULT 'user' AFTER abonnement;
  END IF;
