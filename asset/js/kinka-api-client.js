...
// FETCH DE BASE
// ════════════════════════════════════════════════════════════════
async function kinkaFetch(path, options = {}) {                   // wrapper fetch : ajoute le token + gère les erreurs
  ...
  return json.data;                                             // succès : renvoie la charge utile
}
...
document.addEventListener('DOMContentLoaded', function initCookieBanner() { // bannière cookies au chargement
  ...
  document.getElementById('kinka-cookies-accept').onclick = () => { // clic sur "Accepter"
    KinkaCookies.set('kinka_cookies_accepted', '1', 365);      // mémorise l'accord (1 an)
    banner.remove();                                           // retire la bannière
  };
  document.getElementById('kinka-cookies-decline').onclick = () => { // clic sur "Refuser"
    KinkaCookies.set('kinka_cookies_accepted', '0', 30);       // mémorise le refus (30 j)
    banner.remove();                                           // retire la bannière
  };
});
