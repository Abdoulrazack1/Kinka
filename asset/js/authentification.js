    ...
    resetForm.addEventListener('submit', async function(e) {       // à la soumission
      ...
      try {                                                        // tentative
        if (typeof KinkaAPI !== 'undefined' && KinkaAPI.auth.forgot) { // API dispo
          await KinkaAPI.auth.forgot(email);                       // demande de réinitialisation
        }
        showToast("Si cet email existe, un lien de réinitialisation vous sera envoyé.", 'info'); // message neutre
        resetForm.reset();                                        // réinitialise le formulaire
      } catch (err) {                                              // erreur
        // Même message pour ne pas révéler l'existence d'un email
        showToast("Si cet email existe, un lien de réinitialisation vous sera envoyé.", 'info'); // message identique (anti-énumération)
