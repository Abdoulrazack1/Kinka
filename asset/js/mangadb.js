...
// ============================================================
const categoriesDB = [
    {id:'shonen',nom:'Shōnen',label:'SHŌNEN',image:'/asset/image/catégorie_shonen.webp',description:'Aventure, action, combat et amitié pour tous les âges.',couleur:'#ef4444',filtreCategorie:'Shônen'},
    {id:'seinen',nom:'Seinen',label:'SEINEN',image:'/asset/image/catégorie_seinen.jpg',description:'Histoires matures, psychologiques et réalistes.',couleur:'#6366f1',filtreCategorie:'Seinen'},
    {id:'shojo',nom:'Shōjo',label:'SHŌJO',image:'/asset/image/catégorie_shojo.jpg',description:'Romance, émotions et tranches de vie touchantes.',couleur:'#ec4899',filtreCategorie:'Shôjo'},
...
function filterProducts(opts = {}) {
    ...
    if (opts.editeur)    result = result.filter(m => m.editeur === opts.editeur);
    ...
    if (opts.sort === 'prix_asc')  result.sort((a,b) => a.prix - b.prix);
    if (opts.sort === 'prix_desc') result.sort((a,b) => b.prix - a.prix);
...
function kinkaAddToCart(id, e) {
    ...
    if (typeof window.addToCart === 'function') {
        ...
        if (manga) window.addToCart(manga);
    } else {
        // Fallback direct
        const manga = getMangaById(id);
        if (!manga) return;
        ...
        else panier.push({id, titre: manga.titre, prix: manga.prix, image: manga.image, quantite: 1});
